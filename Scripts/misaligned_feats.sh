...
do
    ...
    # GIN with features
    python Exp/run_experiment.py -grid "Configs/Eval/gin_misaligned_features.yaml" -dataset "${dataset[i]}" --candidates 50  --repeats 1 --graph_feat "Counts/RepeatedCounts/${counts_cur_path[i]}"    
    python Exp/run_multi_count_experiment.py -grid "Configs/Eval/gin_misaligned_features.yaml" -dataset "${dataset[i]}" --repeats 2 --nr_diff_counts 9
    done